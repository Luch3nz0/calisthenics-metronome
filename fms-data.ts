...
export type FmsTask = {
  id: string
  patternKey: FmsPatternKey
  patternName: string
  kind: 'movement' | 'clearing'
  side?: 'right' | 'left'
  equipment: string[]
  cameraView: string
  instructions: string[]
...
type PatternDefinition = {
  ...
  cameraView: string
  instructions: string[]
  buildVoiceScript: (sex: SexForTSPU, side?: 'right' | 'left') => string
  buildScoreOptions: (sex: SexForTSPU) => readonly ScoreOption[]
}
...
export const fmsDisclaimer =
  'This screen does not replace evaluation by a qualified professional such as a physician or personal trainer. It is only a movement-screening tool. If you feel pain during any movement, stop immediately and seek professional evaluation.'

export const fmsVoiceDisclaimer =
  'This screen does not replace a professional opinion. Stop if you feel pain at any time.'

export const fmsOpeningVoice =
  'We are starting the Functional Movement Screen. This does not replace professional medical or training advice. Stop if you feel pain. Complete all seven movements to receive your results.'
...
const genericRightLeftOptions = (score3: string, score2: string, score1: string): readonly ScoreOption[] => [
  {
    value: 3,
    label: 'Score 3',
    description: score3
...
const patternDefinitions: readonly PatternDefinition[] = [
  {
    ...
    buildVoiceScript: () =>
      'Deep Squat. Equipment needed: dowel. Stand with your feet about shoulder width apart and toes forward. Press the dowel overhead. Hold still while I check your position. Good. Squat down as deep as possible, keep your heels down, hold the bottom briefly, and stand back up.',
    buildScoreOptions: () =>
      genericRightLeftOptions(
        'Torso stayed upright or parallel to the tibia, the femur went below horizontal, knees stayed aligned over the feet, and the dowel stayed over the feet.',
        'The pattern was completed only after repeating with the heels elevated on the board.',
        'The pattern could not be completed cleanly even after the heel-elevated repeat.'
      )
  },
  {
    key: 'hurdleStep',
    ...
    buildVoiceScript: (_sex, side) =>
      `Hurdle Step. Equipment needed: dowel and hurdle. Stand tall with your feet together and the dowel across your shoulders. Hold still while I check your position. Good. Lift your ${side ?? 'right'} leg, step over the hurdle, touch the floor with your heel, and return.`,
    buildScoreOptions: () =>
      genericRightLeftOptions(
        'Hips, knees, and ankles stayed aligned, lumbar movement stayed minimal, and balance was controlled.',
        'The step was completed but alignment or trunk control was lost during the movement.',
        'The foot contacted the hurdle or balance was lost.'
      )
  },
  {
    key: 'inlineLunge',
    ...
    buildVoiceScript: () =>
      'Inline Lunge. Equipment needed: dowel and board. Place the dowel along your spine so it touches your head, upper back, and hips. Get your feet into the marked split stance. Hold still while I check your position. Good. Lower into the lunge until your knee touches behind the front heel, then return to standing.',
    buildScoreOptions: () =>
      genericRightLeftOptions(
        'The dowel contacts were maintained, the dowel stayed vertical, the torso stayed stable, and the knee touched behind the front heel.',
        'The lunge was completed but dowel contacts, vertical control, or the knee target were not maintained.',
        'Balance was lost during the lunge.'
      )
  },
  {
    ...
    buildVoiceScript: () =>
      'Shoulder Mobility. Equipment needed: measuring device. Stand tall. Make a fist with both hands. Reach one fist over your shoulder and down your back, and the other up your back in one motion. Do not move your hands closer after placing them. Hold still.',
    buildScoreOptions: () =>
      genericRightLeftOptions(
        'The fists finished within one hand length.',
        'The fists finished within one and a half hand lengths.',
        'The fists were farther apart than one and a half hand lengths.'
      )
  },
  {
    ...
    buildScoreOptions: () =>
      genericRightLeftOptions(
        'The malleolus line reached between the opposite mid-thigh and ASIS with the non-moving leg staying neutral.',
        'The malleolus line reached between the opposite mid-thigh and knee joint line with the non-moving leg staying neutral.',
        'The malleolus line stayed below the opposite knee joint line or the non-moving leg compensated.'
      )
  },
  {
    ...
    buildScoreOptions: (sex) => {
      ...
      return genericRightLeftOptions(
        `The body lifted as one unit with no spinal lag using ${score3Placement}.`,
        `The body lifted as one unit with no spinal lag using ${score2Placement}.`,
        'The push-up could not be completed at the adjusted hand position.'
      )
    }
  },
  {
    ...
    buildVoiceScript: (_sex, side) =>
      `Rotary Stability. Equipment needed: board. Get on your hands and knees over the board. Hold still while I check your position. Good. Reach your ${side === 'left' ? 'left' : 'right'} arm forward and your ${side === 'left' ? 'left' : 'right'} leg backward. Now bring your elbow and knee together over the board, extend again, and return to start.`,
    buildScoreOptions: () =>
      genericRightLeftOptions(
        'A correct same-side repetition was completed.',
        'The same-side repetition failed, but the diagonal regression was completed correctly.',
        'The diagonal regression could not be completed.'
      )
  }
] as const

const clearingInstructions: Record<FmsPatternKey, string[]> = {
  deepSquat: [],
  hurdleStep: [],
  inlineLunge: [],
  shoulderMobility: [
...
export function createEmptyFmsPatterns(): FmsPatternMap {
  return {
...
export function getFmsPatternName(patternKey: FmsPatternKey): string {
  const pattern = patternDefinitions.find((item) => item.key === patternKey)
  return pattern?.name ?? patternKey
}

export function createFmsTasks(sex: SexForTSPU): FmsTask[] {
  const tasks: FmsTask[] = []
    ...
    if (pattern.key === 'shoulderMobility') {
      for (const side of ['right', 'left'] as const) {
        tasks.push({
          ...
          voiceScript:
            'Next, place your hand on the opposite shoulder and lift your elbow. Tell me if you feel any pain.',
      ...
      tasks.push({
        ...
        instructions: clearingInstructions[pattern.key],
        voiceScript:
          'Good. Now perform the spinal extension clearing movement and tell me if you feel pain.',
        painPrompt: clearingPainPrompt,
        scoreOptions: []
      })
      ...
      tasks.push({
        ...
        instructions: clearingInstructions[pattern.key],
        voiceScript:
          'Good. Now perform the spinal flexion clearing movement and tell me if you feel pain.',
        painPrompt: clearingPainPrompt,
        scoreOptions: []
      })
    }
...
export function computeFmsOutcome(patterns: FmsPatternMap): {
  complete: boolean
  totalScore?: number
  anyPain: boolean
  anyAsymmetry: boolean
  lowestPatterns: FmsPatternKey[]
  needsProfessionalFollowup: boolean
} {
  const scores = fmsPatternKeys
    .map((patternKey) => ({ patternKey, score: patterns[patternKey].finalScore }))
    .filter((entry): entry is { patternKey: FmsPatternKey; score: FmsScoreValue } => entry.score !== undefined)
